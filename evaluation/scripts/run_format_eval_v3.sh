#!/bin/bash
# Format Correctness Evaluation (BFCL v3) for all 23 models (base + 22 checkpoints)
# Uses offline vLLM batch inference - no server needed
#
# Usage: bash scripts/run_format_eval_v3.sh <GPU_ID>
...
OUTPUT_DIR="${OUTPUT_DIR:-/data/sxw240003/GDPO/eval_results/toolrl_dgdo2_v3/format}"
# ====================================

GPU_ID=${1:?Usage: bash scripts/run_format_eval_v3.sh <GPU_ID (0-3)>}
...
echo "GPU $GPU_ID (BFCL v3): ${VARIANTS[*]}"
...
for variant in "${VARIANTS[@]}"; do
    ...
    echo "=========================================================="
    echo "Format Eval (BFCL v3): $model_id (GPU $GPU_ID)"
    echo "=========================================================="
...
echo "GPU $GPU_ID: All BFCL v3 format evaluations finished!"
