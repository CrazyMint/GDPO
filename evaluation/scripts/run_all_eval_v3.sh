#!/bin/bash
# ===== BFCL v3 Combined Evaluation: AST Accuracy + Format Correctness =====
# Evaluates all 23 models (base + 22 checkpoints).
# For each model: runs format eval once, then AST eval 5 times.
# Split across 4 GPUs for parallelism.
#
# Usage: bash scripts/run_all_eval_v3.sh <GPU_ID>
...
GPU_ID=${1:?Usage: bash scripts/run_all_eval_v3.sh <GPU_ID (0-3)>}
...
echo "GPU $GPU_ID (BFCL v3 Combined Eval): ${VARIANTS[*]}"
echo ""
...
for variant in "${VARIANTS[@]}"; do
    ...
    # --- Format Eval (1 run) ---
    if [ -f "$FORMAT_DIR/${model_id}_format_summary.json" ]; then
        echo "  [Format] SKIP: already evaluated"
    else
        echo "  [Format] Running format eval..."
        python "$EVAL_ROOT/bfcl_v3/format_eval.py" \
    ...
    # --- AST Accuracy Eval (5 runs) ---
    for run_id in $(seq 1 $NUM_RUNS); do
        output_path="$OUTPUT_DIR/tool/run_$run_id/$model_id"

        if [ -d "$output_path/score" ]; then
            echo "  [AST Run $run_id] SKIP: already has score/"
            continue
        fi

        echo "  [AST Run $run_id] Running..."
        python "$EVAL_ROOT/bfcl_v3/tool_eval.py" \
            --model_path "$model_path" \
            --model_id "$model_id" \
            --output_dir "$output_path" \
            --force_reregister
        echo "  [AST Run $run_id] Done"
    done

    echo ""
done

echo "GPU $GPU_ID: All BFCL v3 evaluations finished!"
