#!/bin/bash
# ===== BFCL v3 AST Tool Calling Evaluation =====
# Evaluates all 23 models (base + 22 checkpoints) with 5 runs each.
# Split across 4 GPUs for parallelism.
#
# Usage: bash scripts/run_tool_eval_v3.sh <GPU_ID>
...
GPU_ID=${1:?Usage: bash scripts/run_tool_eval_v3.sh <GPU_ID (0-3)>}
...
echo "GPU $GPU_ID (BFCL v3 AST Eval): ${VARIANTS[*]}"
...
for variant in "${VARIANTS[@]}"; do
    ...
    echo "=========================================================="
    echo "[GPU $GPU_ID] AST Eval (BFCL v3): $model_id"
    echo "=========================================================="
    ...
    for run_id in $(seq 1 $NUM_RUNS); do
        ...
        if [ -d "$output_path/score" ]; then
            echo "  SKIP Run $run_id: already has score/ directory"
            continue
        fi
...
echo "GPU $GPU_ID: All BFCL v3 AST evaluations finished!"
