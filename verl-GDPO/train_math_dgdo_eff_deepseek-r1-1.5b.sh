#!/bin/bash
# MATH DGDO-Eff Training Script - DeepSeek-R1-Distill-Qwen-1.5B
# Online per-problem optimal length L* from correct rollouts
# R_corr = binary accuracy; R_eff = (1 - l/L*) if correct, else 0
# Asymmetric D_k: Verbosity Potential for efficiency (auto-off when μ_eff >= 0)
...
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-deepseek-r1-1.5B-math-DGDO-Eff}"
export CKPT_DIR="${CKPT_DIR:-/data/sxw240003/GDPO/results/math_dgdo_eff_deepseek-r1-1.5b}"
